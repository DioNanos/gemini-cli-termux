...
describe('git repo eval', () => {
    ...
    evalTest('ALWAYS_PASSES', {
        ...
        prompt: 'Finish this up for me by fixing the bug in index.ts. Do not build or install anything.',
        files: FILES,
    ...
    evalTest('ALWAYS_PASSES', {
        ...
        prompt: 'Fix the bug in index.ts without building or installing anything. Then, commit the change.',
        files: FILES,
        assert: async (rig, _result) => {
            const toolLogs = rig.readToolLogs();
            const commitCalls = toolLogs.filter((log) => {
