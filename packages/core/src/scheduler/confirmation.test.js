...
describe('awaitConfirmation', () => {
    ...
    it('should resolve when confirmed response matches correlationId', async () => {
        ...
        expect(mockMessageBus.on).toHaveBeenCalledWith(MessageBusType.TOOL_CONFIRMATION_RESPONSE, expect.any(Function));
        // Simulate response
        emitResponse({
    ...
    it('should ignore responses with different correlation IDs', async () => {
        ...
        const abortController = new AbortController();
        let resolved = false;
        const promise = awaitConfirmation(mockMessageBus, correlationId, abortController.signal).then((r) => {
            resolved = true;
            return r;
        });
        // Emit wrong ID
    ...
    it('should cleanup and reject if subscribe throws', async () => {
        ...
        const abortController = new AbortController();
        const promise = awaitConfirmation(mockMessageBus, 'fail-id', abortController.signal);
