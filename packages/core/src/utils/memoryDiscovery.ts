...
export async function loadServerHierarchicalMemory(
  ...
  // TERMUX PATCH: Load JSON context memory if enabled
  let contextMemoryPaths: string[] | undefined;
  if (contextMemoryOptions?.enabled) {
    const geminiInstructions = concatenateInstructions(
      allContents.filter((c) => c.content !== null),
      currentWorkingDirectory,
    );
