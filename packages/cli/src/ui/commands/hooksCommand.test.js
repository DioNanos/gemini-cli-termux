...
describe('hooksCommand', () => {
    ...
    describe('panel subcommand', () => {
        ...
        it('should display panel when no hooks are configured', async () => {
            mockHookSystem.getAllHooks.mockReturnValue([]);
            mockContext.services.settings.merged['tools'] = { enableHooks: true };
            const panelCmd = hooksCommand.subCommands.find((cmd) => cmd.name === 'panel');
            if (!panelCmd?.action) {
                throw new Error('panel command must have an action');
            }
    ...
    describe('enable-all subcommand', () => {
        it('should return error when config is not loaded', async () => {
            ...
            const enableAllCmd = hooksCommand.subCommands.find((cmd) => cmd.name === 'enable-all');
        ...
        it('should enable all disabled hooks', async () => {
            ...
            const result = await enableAllCmd.action(mockContext, '');
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(expect.any(String), 'hooks.disabled', []);
    ...
    describe('disable-all subcommand', () => {
        it('should return error when config is not loaded', async () => {
            ...
            const disableAllCmd = hooksCommand.subCommands.find((cmd) => cmd.name === 'disable-all');
        ...
        it('should disable all enabled hooks', async () => {
            ...
            const result = await disableAllCmd.action(mockContext, '');
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(expect.any(String), 'hooks.disabled', ['hook-1', 'hook-2', 'hook-3']);
