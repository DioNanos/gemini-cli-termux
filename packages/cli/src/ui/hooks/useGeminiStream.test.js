...
describe('useGeminiStream', () => {
    ...
    describe('MCP Discovery State', () => {
        it('should block non-slash command queries when discovery is in progress and servers exist', async () => {
            ...
            mockConfig.getMcpClientManager = () => mockMcpClientManager;
            const { result } = renderTestHook();
            await act(async () => {
                await result.current.submitQuery('test query');
            });
            expect(coreEvents.emitFeedback).toHaveBeenCalledWith('info', 'Waiting for MCP servers to initialize... Slash commands are still available.');
            expect(mockSendMessageStream).not.toHaveBeenCalled();
        });
        it('should NOT block queries when discovery is NOT_STARTED but there are no servers', async () => {
            ...
            expect(coreEvents.emitFeedback).not.toHaveBeenCalledWith('info', 'Waiting for MCP servers to initialize... Slash commands are still available.');
